...
public class Board {
	...
	public Board clone(Square[][] BOARD) {
	...
	public void moveCustom(Person person, int x, int y, int x2, int y2) {
		Optional<Token> opTok;
		Movement m;
		int x1, y1, w, z;

		m = person.confirmMovePieceCustom(x, y, x2, y2);

		x1 = m.getSquareO().getPosition().getX();
		y1 = m.getSquareO().getPosition().getY();
		w = m.getSquareD().getPosition().getX();
		z = m.getSquareD().getPosition().getY();

		BOARD[x1][y1].returnToken().get().setSquare(BOARD[w][z]);
		opTok = BOARD[x1][y1].returnToken();
		BOARD[w][z].setToken(opTok.get());
		BOARD[x1][y1].setToken(null);
